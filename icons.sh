...
for f in ${icons[@]}; do
	echo -e "\nConverting $f..."
	inkscape --export-png icons/$f$size.png -h $size -w $size icons/$f.svg
	inkscape --export-png icons/$f$size_hidpi.png -h $size_hidpi -w $size_hidpi icons/$f.svg
done

echo -e "\nConverting application icon to png..."
inkscape --export-png icons/pyhsi.png -h 256 -w 256 icons/pyhsi.svg

echo -e "\nConverting application icon to ico..."
mkdir tmp
declare -a sizes=(16 32 48 128 256)
for s in ${sizes[@]}; do
	inkscape --export-png tmp/$s.png -h $s -w $s icons/pyhsi.svg
done
